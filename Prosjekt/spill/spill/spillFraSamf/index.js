...
var bildeArr = ["img/leftLegTo.png", "img/RightLeg.png"]; //array med bildene til karakter
...
btnMeny.onclick = function startGame() {
  //knapp forsvinner
  btnMeny.style.display = "none";
  //komponent til karakter
  karakter = new karakterKomponent(
    50,
    80,
    "img/leftLeg.png",
    240,
  ...
  spillbakgrunn = new spillbakgrunnKomponent(
    450,
    600,
    "img/pavement.jpg",
    0,
  ...
  poengsum = new poengsumKomponent(
    "30px",
    "Consolas",
    "black",
    105,
...
function poengsumKomponent(width, height, color, x, y, type) {
  ...
  this.update = function() {
    ...
    if (this.type == "text") {
      ...
      //hvis ikke greier å utføre så fylles det blankt
      ctx.fillStyle = color;
...
function karakterKomponent(width, height, color, x, y, type) {
  ...
  function bytte() {
    ...
    i = (i + 1) % bildeArr.length; //ender verdien til i mellom 0 og 1
  }
...
function oppdaterSpillOmraade() {
  ...
  for (i = 0; i < Hindringer.length; i += 1) {
    if (karakter.crashWith(Hindringer[i])) {
      ...
      document.getElementById("karakter").src =
        "../../karakterbilder/trist.png";
  ...
  var scoreNedtelling = Math.ceil(200 - spillomraade.frameNo / 10);
